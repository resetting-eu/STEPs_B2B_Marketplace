// "use strict";
function mainHoriz2() {
  const svg = d3.select("#chart_horiz2");
  const chart = svg
    .attr("id", "chart_horiz2")
    .append("g")
  ...
  defs
    .append("clipPath")
    .attr("id", "trianglePath_horiz2")
    .append("path")
    .attr("d", _textureTernaryPlot.triangle());

  defs
    .append("marker")
    .attr("id", "arrow_horiz2")
    .attr("markerWidth", "10")
  ...
  const axisLabelsGroups = axisLabelsGroup
    ...
    .join(
      (enter) => {
        ...
        const axisArrow_horiz2 = labelGroup
          .append("line")
          .attr("stroke", "black")
          .attr("x1", (d, i) => (i === 2 ? 100 : -100))
          .attr("x2", (d, i) => (i === 2 ? -100 : 100))
          .attr("marker-end", "url(#arrow_horiz2)");
  ...
  const siltCell = res_display_table_horiz2.rows[1].cells[0];
  const clayCell = res_display_table_horiz2.rows[1].cells[1];
  const sandCell = res_display_table_horiz2.rows[1].cells[2];
