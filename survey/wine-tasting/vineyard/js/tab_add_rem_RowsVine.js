function hide_and_uncheck_radios_on_val_0_TABLE(nrBox) {
...
if (document.getElementById(radiosContainerID)) {
  document.getElementById(radiosContainerID).hidden = false;
}
if (document.getElementById(parentContainerName)) {
  document.getElementById(parentContainerName).hidden = true;
}

if (document.getElementsByName(radiosName)) {
...
//       // var last_stategkjg = 0; will become a hidden_input_nr_element
          ...
          function tableAdd(addrow, table, hidden_input_nr_element) {
        ...
        return !span.closest('svg');
                      ...
                      if (spansNotInSvg.length > 0) {
                        for(let spa of spansNotInSvg) {
                         // console.log('spa',spa)
                if (spa && spa.innerText.split(' ').join('')[1] != 'ⓘ' && /^\d+$/.test(spa.innerText.split(' ')[spa.innerText.split(' ').length - 1])) {
    ...
    var inputsNotInSvg = Array.from(inputs).filter(function(input) {
        return !input.closest('svg');
                      ...
                      if (inputsNotInSvg.length > 0) {
                        for(var inp of inputsNotInSvg) {
                         // console.log('inp', inp)
                      if (inp && inp.type != "radio") {
                          var new_inp_ID = inp.id.split('_').slice(0, -1).join('_') + "_" + String(i);
                          inp.id = new_inp_ID;
                          inp.value = null;
                      }
                        }
        ...
        return !select.closest('svg');
                      ...
                      if (selectsNotInSvg.length > 0) {
                        for(let sel of selectsNotInSvg) {
                         // console.log('sel', sel)
                          var new_sel_ID = sel.id.split('_').slice(0, -1).join('_') + "_" + String(i);
                          sel.id = new_sel_ID;
                          sel.value = null;
                          sel.removeAttribute('value');
                          // get the options of the select element
                          var options = sel.getElementsByTagName('option');
                          // if any option has attribute 'selected' === true, set it to false
        ...
        return !button.closest('svg');
                      ...
                      if (buttonsNotInSvg.length > 0) {
                        for(let but of buttonsNotInSvg) {
                         // console.log('but', but)
                          var new_but_ID = but.id.split('_').slice(0, -1).join('_') + "_" + String(i);
                          but.id = new_but_ID;
                        }
                      }
        ...
        return !table.closest('svg');
                      ...
                      if (tablesNotInSvg.length > 0) {
                        for(let tab of tablesNotInSvg) {
                         // console.log('tab', tab)
                          var new_tab_ID = tab.id.split('_').slice(0, -1).join('_') + "_" + String(i);
                          tab.id = new_tab_ID;
                        }
                      }
        ...
        return !range.closest('svg');
                      ...
                      if (rangesNotInSvg.length > 0) {
                        for(let ran of rangesNotInSvg) {
                         // console.log('ran', ran)
                           var new_ran_ID = ran.id.split('_').slice(0, -1).join('_') + "_" + String(i);
                           ran.id = new_ran_ID;
                           ran.value = null;
                        }
                      }
        ...
        return !label.closest('svg');
                      ...
                      if (labelsNotInSvg.length > 0) {
                        for (var n = 0; n < labelsNotInSvg.length; n++) {
                          if (inputsNotInSvg[n] && inputsNotInSvg[n].type == "radio") {                            
                         // console.log('inputsNotInSvg[n]', inputsNotInSvg[n])
                            var new_rad_name = inputsNotInSvg[n].name.split('_').slice(0, -1).join('_') + "_" + String(i);
                            var new_rad_ID = inputsNotInSvg[n].id.split('_').slice(0, -1).join('_') + "_" + String(i);
                ...
                inputsNotInSvg[n].checked = false;
                          ...
                          // if the element has attribute 'id'
                          if (labelsNotInSvg[n].hasAttribute('id')) {
                            var new_lab_ID = labelsNotInSvg[n].id.split('_').slice(0, -1).join('_') + "_" + String(i);
                            labelsNotInSvg[n].id = new_lab_ID; 
                          }
                          // if the element has attribute 'for'
                          if (labelsNotInSvg[n].hasAttribute('for')) {
                            var new_lab_for = labelsNotInSvg[n].htmlFor.split('_').slice(0, -1).join('_') + "_" + String(i);
                            labelsNotInSvg[n].htmlFor = new_lab_for;
                          }
                        }
                        ...
                        for (var y = 0; y < inputsNotInSvg.length; y++) {
                          if (inputsNotInSvg[y] && inputsNotInSvg[y].type == 'number') {
                            ...
                            nrBoxi.id = inputsNotInSvg[y].name.split('_').slice(0, -1).join('_') + "_" + String(i);
                            nrBoxi.value = null;
                      ...
                      if (divsNotInSvg.length > 0) {
                        for(let div of divsNotInSvg) {
                         // console.log('div', div)
                          ...
                          var new_div_ID = div.id.split('_').slice(0, -1).join('_') + "_" + String(i);
                          div.id = new_div_ID;
                        }
                      }
        ...
        return !a.closest('svg');
                      ...
                      if (asNotInSvg.length > 0) {
                        for(let a of asNotInSvg) {
                         // console.log('a', a)
                          var new_a_ID = a.id.split('_').slice(0, -1).join('_') + "_" + String(i);
                          a.id = new_a_ID;
             ...
             // console.log('amount_radios',amount_radios)
              ...
              if (amount_radios.length > 0) {
                for (let x = 0; x < amount_radios.length; x++) {
                  hide_and_uncheck_radios_on_val_0_TABLE(document.getElementById(amount_radios[x] + String(i)))
                }
              }
          ...
          function table_remove_row(row, hidden_input_nr_element, addrow) {
    ...
    var inputsNotInSvg = Array.from(inputs).filter(function(input) {
        return !input.closest('svg');
                      ...
                      if (inputsNotInSvg.length > 0) {
                        for(var inp of inputsNotInSvg) {
                      if (inp && inp.type != "radio") {
                          var new_inp_ID = inp.id.split('_').slice(0, -1).join('_') + "_" + String(k - (table.tHead.rows.length - 1));
                          inp.id = new_inp_ID;
                      }
                        }
                      }
    ...
    var selectsNotInSvg = Array.from(selects).filter(function(select) {
        return !select.closest('svg');
                      ...
                      if (selectsNotInSvg.length > 0) {
                        for(let sel of selectsNotInSvg) {
                          var new_sel_ID = sel.id.split('_').slice(0, -1).join('_') + "_" + String(k - (table.tHead.rows.length - 1));
                          sel.id = new_sel_ID;
                        }
                      }
    ...
    var buttonsNotInSvg = Array.from(buttons).filter(function(button) {
        return !button.closest('svg');
                      ...
                      if (buttonsNotInSvg.length > 0) {
                        for(let but of buttonsNotInSvg) {
                          var new_but_ID = but.id.split('_').slice(0, -1).join('_') + "_" + String(k - (table.tHead.rows.length - 1));
                          but.id = new_but_ID;
                        }
                      }
    ...
    var tablesNotInSvg = Array.from(tables).filter(function(table) {
        return !table.closest('svg');
                      ...
                      if (tablesNotInSvg.length > 0) {
                        for(let tab of tablesNotInSvg) {
                          var new_tab_ID = tab.id.split('_').slice(0, -1).join('_') + "_" + String(k - (table.tHead.rows.length - 1));
                          tab.id = new_tab_ID;
                        }
                      }
    ...
    var rangesNotInSvg = Array.from(ranges).filter(function(range) {
        return !range.closest('svg');
                      ...
                      if (rangesNotInSvg.length > 0) {
                        for(let ran of rangesNotInSvg) {
                           var new_ran_ID = ran.id.split('_').slice(0, -1).join('_') + "_" + String(k - (table.tHead.rows.length - 1));
                           ran.id = new_ran_ID;
                        }
                      }
    ...
    var textareasNotInSvg = Array.from(textareas).filter(function(textarea) {
        return !textarea.closest('svg');
                      ...
                      if (textareasNotInSvg.length > 0) {
                        for(let tex of textareasNotInSvg) {
                          var new_textarea_ID = tex.id.split('_').slice(0, -1).join('_') + "_" + String(k - (table.tHead.rows.length - 1));
                          tex.id = new_textarea_ID;
                        }
                      }
    ...
    var labelsNotInSvg = Array.from(labels).filter(function(label) {
        return !label.closest('svg');
                      ...
                      if (labelsNotInSvg.length > 0) {
                        ...
                        for (var n = 0; n < labelsNotInSvg.length; n++) {
                          if (inp[n] && inp[n].type == "radio") {
                            var new_rad_name = inp[n].name.split('_').slice(0, -1).join('_') + "_" + String(k - (table.tHead.rows.length - 1));
                            var new_rad_ID = inp[n].id.split('_').slice(0, -1).join('_') + "_" + String(k - (table.tHead.rows.length - 1));
                        ...
                        // if the element has attribute 'id'
                        if (labelsNotInSvg[n].hasAttribute('id')) {
                          var new_lab_ID = labelsNotInSvg[n].id.split('_').slice(0, -1).join('_') + "_" + String(k - (table.tHead.rows.length - 1));
                          labelsNotInSvg[n].id = new_lab_ID; 
                        }
                        // if the element has attribute 'for'
                        if (labelsNotInSvg[n].hasAttribute('for')) {
                          var new_lab_for = labelsNotInSvg[n].htmlFor.split('_').slice(0, -1).join('_') + "_" + String(k - (table.tHead.rows.length - 1));
                          labelsNotInSvg[n].htmlFor = new_lab_for;
                        }
                      }
                      }
    ...
    var divsNotInSvg = Array.from(divs).filter(function(div) {
        return !div.closest('svg');
                      ...
                      if (divsNotInSvg.length > 0) {
                        for(let div of divsNotInSvg) {
                          var new_div_ID = div.id.split('_').slice(0, -1).join('_') + "_" + String(k - (table.tHead.rows.length - 1));
                          div.id = new_div_ID;
                        }
                      }
    ...
    var asNotInSvg = Array.from(as).filter(function(a) {
        return !a.closest('svg');
                      ...
                      if (asNotInSvg.length > 0) {
                        for(let a of asNotInSvg) {
                          var new_a_ID = a.id.split('_').slice(0, -1).join('_') + "_" + String(k - (table.tHead.rows.length - 1));
                          a.id = new_a_ID;
                        }
                      }
    ...
    var inputsNotInSvg = Array.from(inputs).filter(function(input) {
        return !input.closest('svg');
                      ...
                      if (inputsNotInSvg.length > 0) {
                        for(var inp of inputsNotInSvg) {
                      if (inp && inp.type != "radio") {
                          var new_inp_ID = inp.id.split('_').slice(0, -1).join('_') + "_" + String(1);
                          inp.id = new_inp_ID;
                          inp.value = null;
                      }
                        }
    ...
    var selectsNotInSvg = Array.from(selects).filter(function(select) {
        return !select.closest('svg');
                      ...
                      if (selectsNotInSvg.length > 0) {
                        for(let sel of selectsNotInSvg) {
                          var new_sel_ID = sel.id.split('_').slice(0, -1).join('_') + "_" + String(1);
                          sel.id = new_sel_ID;
                          sel.value = null;
                          sel.removeAttribute('value');
                          // get the options of the select element
    ...
    var buttonsNotInSvg = Array.from(buttons).filter(function(button) {
        return !button.closest('svg');
                      ...
                      if (buttonsNotInSvg.length > 0) {
                        for(let but of buttonsNotInSvg) {
                          var new_but_ID = but.id.split('_').slice(0, -1).join('_') + "_" + String(1);
                          but.id = new_but_ID;
                        }
                      }
    ...
    var tablesNotInSvg = Array.from(tables).filter(function(table) {
        return !table.closest('svg');
                      ...
                      if (tablesNotInSvg.length > 0) {
                        for(let tab of tablesNotInSvg) {
                          var new_tab_ID = tab.id.split('_').slice(0, -1).join('_') + "_" + String(1);
                          tab.id = new_tab_ID;
                        }
                      }
    ...
    var rangesNotInSvg = Array.from(ranges).filter(function(range) {
        return !range.closest('svg');
                      ...
                      if (rangesNotInSvg.length > 0) {
                        for(let ran of rangesNotInSvg) {
                           var new_ran_ID = ran.id.split('_').slice(0, -1).join('_') + "_" + String(1);
                           ran.id = new_ran_ID;
                           ran.value = null;
                        }
                      }
    ...
    var textareasNotInSvg = Array.from(textareas).filter(function(textarea) {
        return !textarea.closest('svg');
                      ...
                      if (textareasNotInSvg.length > 0) {
                        for(let tex of textareasNotInSvg) {
                          var new_textarea_ID = tex.id.split('_').slice(0, -1).join('_') + "_" + String(1);
                          tex.id = new_textarea_ID;
                        }
                      }
    ...
    var labelsNotInSvg = Array.from(labels).filter(function(label) {
        return !label.closest('svg');
                      ...
                      if (labelsNotInSvg.length > 0) {
                        for (var n = 0; n < labelsNotInSvg.length; n++) {
                          if (inputsNotInSvg[n] && inputsNotInSvg[n].type == "radio") {
                            var new_rad_name = inputsNotInSvg[n].name.split('_').slice(0, -1).join('_') + "_" + String(1);
                            var new_rad_ID = inputsNotInSvg[n].id.split('_').slice(0, -1).join('_') + "_" + String(1);
                            inputsNotInSvg[n].name = new_rad_name;
                            inputsNotInSvg[n].id = new_rad_ID;
                            // inputsNotInSvg[n].checked = false;
                          ...
                          // if the element has attribute 'id'
                          if (labelsNotInSvg[n].hasAttribute('id')) {
                            var new_lab_ID = labelsNotInSvg[n].id.split('_').slice(0, -1).join('_') + "_" + String(1);
                            labelsNotInSvg[n].id = new_lab_ID; 
                          }
                          // if the element has attribute 'for'
                          if (labelsNotInSvg[n].hasAttribute('for')) {
                            var new_lab_for = labelsNotInSvg[n].htmlFor.split('_').slice(0, -1).join('_') + "_" + String(1);
                            labelsNotInSvg[n].htmlFor = new_lab_for;
                          }
                        }
                        // resetting the storage nrBox
                        for (var y = 0; y < inputsNotInSvg.length; y++) {
                          if (inputsNotInSvg[y] && inputsNotInSvg[y].type == 'number') {
                            var nrBoxi = inputsNotInSvg[y];
                            nrBoxi.id = inputsNotInSvg[y].name.split('_').slice(0, -1).join('_') + "_" + String(1);
                            nrBoxi.value = null;
                            amount_radios.push(inputsNotInSvg[y].name.split('_').slice(0, -1).join('_') + "_");                  
                          }
                        }					
    ...
    var divsNotInSvg = Array.from(divs).filter(function(div) {
        return !div.closest('svg');
                      ...
                      if (divsNotInSvg.length > 0) {
                        for(let div of divsNotInSvg) {
                          var new_div_ID = div.id.split('_').slice(0, -1).join('_') + "_" + String(1);
                          div.id = new_div_ID;
                        }
                      }
    ...
    var asNotInSvg = Array.from(as).filter(function(a) {
        return !a.closest('svg');
                      ...
                      if (asNotInSvg.length > 0) {
                        for(let a of asNotInSvg) {
                          var new_a_ID = a.id.split('_').slice(0, -1).join('_') + "_" + String(1);
                          a.id = new_a_ID;
                        }
                      }
                  }
                  // // calling the storage number box 'change function on the null value set above
                  // hide_and_uncheck_radios_on_val_0_TABLE(document.getElementById('winery_externalProvidorStorage_Amount_1')); 
              // calling the storage number box 'change function on the null value set above
              if (amount_radios.length > 0) {
                for (let x = 0; x < amount_radios.length; x++) {
                  hide_and_uncheck_radios_on_val_0_TABLE(document.getElementById(amount_radios[x] + String(1)))
                }
              }
